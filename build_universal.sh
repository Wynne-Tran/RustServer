...
OUT_DIR="ios_server/include"
XCFRAMEWORK_DIR="ios_server.xcframework"

mkdir -p $OUT_DIR
if [ ! -f "$OUT_DIR/ios_server.h" ]; then
  echo "📝 Generating header file..."
  cd ios_server
  cargo install --force cbindgen  # Ensure cbindgen is installed
  cbindgen --lang c --output include/ios_server.h
  cd ..
else
...
xcodebuild -create-xcframework \
  -library target/aarch64-apple-ios/release/libios_server.a -headers ios_server/include/ \
  -library target/universal-ios-simulator.a -headers ios_server/include/ \
